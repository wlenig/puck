...
<ConfigPreview
  ...
  componentConfig={{
    ...
    defaultProps: {
      richtext:
        "<p style='text-align: center;'><strong>✍️ Hello,<s>Plain-text</s> Rich Text! 🎉</strong></p>",
    },
  }}
/>
...
<Callout>
    Note: The `e.stopPropagation();` is required in Puck in order to avoid issues with the portal on inlineMenu.
</Callout>

### `textSelectOptions`

Configure the block-style dropdown rendered in the toolbar. Supply any subset of `"p" | "h1" … "h6"` to narrow or expand the list.
